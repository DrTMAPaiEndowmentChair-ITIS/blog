...
topics: ['LoRA', 'Multi-tenancy', 'LLM serving']
---

import AdapterMultiplexing from '../../components/viz/AdapterMultiplexing.astro'

Fine-tuning produces adapters far more often than it produces full models. A LoRA adapter for a 70B model is often under 200 MB against 140 GB of base weights. The economics of that ratio only materialize if one replica can serve many adapters at once. If each adapter requires its own replica, a 700x size advantage becomes a 1x cost advantage.

Getting from one to the other is a scheduling and kernel problem, not a training one.

<AdapterMultiplexing />

## The structure that makes multiplexing possible
...
Merged serving is the fastest possible per-token path — it is exactly base-model inference, no extra kernels, no extra memory traffic. It also destroys multiplexing. A merged replica serves exactly one adapter, and switching means either re-merging (a full pass over the weights) or keeping a second copy.
...
The headline claim for these systems is throughput at high adapter counts. The number that predicts whether a deployment is actually viable is the p99 TTFT of the least-popular tenant on the replica, because that is the one whose adapter is never resident and whose requests are never the locality-preferred choice.
