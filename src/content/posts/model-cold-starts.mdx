...
topics: ['Cold start', 'Weight loading', 'Autoscaling']
---

import WeightLoadPath from '../../components/viz/WeightLoadPath.astro'

Serving benchmarks report tokens per second on a warm replica. Autoscaling behavior is decided by a different number: how long a replica takes to go from scheduled to serving. For a large model that number is dominated by moving tens or hundreds of gigabytes through a chain of increasingly fast, increasingly small memories.

Cold start is a bandwidth problem with a scheduling problem layered on top. Both are measurable, and most deployments are slow for reasons that have nothing to do with the model.

<WeightLoadPath />

## The chain and its rates
...
For a 140 GiB checkpoint at 3 GiB/s network and 20 GiB/s effective host-to-device, that is roughly 47 + 7 = 54 seconds. Deployments routinely see three to five times that, and the gap is almost always one of a small number of causes.
...
**Serialized stages.** Fetch, then copy, then transfer, with nothing overlapping. The stages use different resources — NIC, CPU, PCIe — and should be pipelined. Overlapping the network fetch of chunk `n+1` with the device transfer of chunk `n` brings the total close to `S / min(B_net, B_h2d)` rather than the sum.
...
**A warm benchmark measures nothing about cold start.** Drop caches, or measure on a genuinely fresh node, or the result is a page-table exercise.
...
That allows a genuine pipeline: begin prefill as soon as the early layers land, and continue streaming later layers while early layers compute. The gain is bounded by the ratio of compute time to transfer time, which for a single prefill is usually unfavorable — the transfer is much slower — so this mostly helps the first request rather than the replica's readiness overall.
...
Cold start is not a fixed property of a model. It is a property of the path, and every hop in that path is something you chose.
