...
topics: ['SLO', 'Latency', 'Benchmarking']
---
...
Throughput is $n / T_{\text{step}}(n)$ and TPOT is $T_{\text{step}}(n)$. At small $n$ the constant weight term dominates, so doubling $n$ nearly doubles throughput at nearly unchanged TPOT — this is free money and the reason batching exists. As $n$ grows, the KV term and then compute saturation make $T_{\text{step}}$ grow linearly, and throughput flattens while TPOT keeps climbing.
...
At $\rho = 0.5$, wait is one service time. At $\rho = 0.9$, nine. At $\rho = 0.95$, nineteen. LLM serving is not M/M/1 — service times are heavy-tailed because output lengths are — but the shape holds and is worse: high service-time variance amplifies the tail further.
...
Peak throughput remains a useful number — it bounds what is possible. But nothing in a deployment should be tuned to it, because it is achieved at an operating point where a large fraction of requests are already failing their objective.
