...
topics: ['Quantization', 'Architecture', 'Training']
---

import PrecisionFormatLadder from '../../components/viz/PrecisionFormatLadder.astro'

Quantization is usually applied after the fact: train in BF16, compress, measure the damage. That framing treats low precision as a lossy postprocessing step and leaves out the model's own structure, which decides how much damage there is.
...
| INT8     | 8    | none     | 7 + sign | scale-dependent | uniform            |
| INT4     | 4    | none     | 3 + sign | scale-dependent | uniform            |
...
At $b = 4$ and $g = 128$, that is 4.25 bits. At $g = 32$, 5 bits, a 25% overhead. That is the range where "4-bit" and "5-bit" quantizations stop being clearly ordered.
