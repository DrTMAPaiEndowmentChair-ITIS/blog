...
topics: ['Model hub', 'Distribution', 'Supply chain']
---

import HubTransferPath from '../../components/viz/HubTransferPath.astro'

Most teams meet a model hub as a convenience function — one call, weights appear. That framing works until the download is 300 GB, the fleet is 200 nodes, the CI job needs to be reproducible next quarter, and someone asks who signed off on the bytes.

At that point the hub stops being a convenience and becomes part of the deployment path, with the same properties you would demand of a package registry: content addressing, deduplication, pinning, verification, and an offline mode.
...
The hub is genuinely good infrastructure — content addressing, chunk dedup, and resumable transfer are the right primitives. The mistake is treating it as a filesystem that happens to be remote instead of as a registry that happens to be public.
