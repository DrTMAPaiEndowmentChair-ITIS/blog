...
import ComputeBudgetAllocation from '../../components/viz/ComputeBudgetAllocation.astro'

Scaling laws fit a small equation to measurements from many training runs. The fitted curve can predict loss at an untested scale and guide the allocation of a fixed training budget. Its domain is limited by the model family, data distribution, optimizer, tokenization scheme, and measurement range used to produce it.

Read the equation with its conditions attached:
...
A 2026 preprint proposes SoftQ, a coupled model of parameter count and data size for repeated-data training, arguing that the additive Chinchilla form can be misspecified in this regime.[^5] The paper evaluates SoftQ for repeated-data training.
...
A scaling fit supports the measured relationship for one family of systems over one range. Researchers can use that fit to avoid some experimental runs. Extending it to new architectures, repeated data, post-training, test-time reasoning, or user value requires new measurements.
