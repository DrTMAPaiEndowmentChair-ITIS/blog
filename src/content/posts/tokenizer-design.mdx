...
topics: ['Tokenizers', 'Vocabulary', 'Architecture']
---
...
For a component with those properties, it receives remarkably little design attention.
...
For English, a well-fitted 32k BPE vocabulary lands around 1.3. For languages written in scripts underrepresented in the training corpus, the same tokenizer can produce 3 to 5. This is not a small inefficiency — it means the same semantic content costs several times more, consumes several times more of the context window, and is billed several times more.
...
**Unigram** starts from a large candidate set and prunes to maximize corpus likelihood under a unigram model, which yields a probabilistic segmentation and enables subword regularization — sampling different segmentations during training as an augmentation.
...
4. **Reserve special tokens generously.** Adding them later means resizing the embedding, and running out mid-project is a genuinely annoying problem.
