...
That distinction explains why two files described as the same model can have different memory requirements, why an accelerator can sit idle while inference uses the CPU, and why a checkpoint larger than available RAM can still produce tokens slowly through disk offload.

This guide separates those variables. The goal is not to produce a universal compatibility list. It is to give you a repeatable way to estimate a configuration before downloading it, then identify the next constraint when the estimate does not fit.
...
The division by eight converts bits to bytes. This is a lower bound for a uniformly encoded checkpoint, not a prediction of the final file size.
...
The diagram distinguishes capacity from placement. A checkpoint can fit on storage but not in system RAM. It can fit in RAM but not in GPU memory. It can fit across RAM and GPU memory while spending substantial time moving tensors over the interconnect.
...
Context settings also affect generation length. The cache contains prompt tokens plus generated tokens that remain inside the active context. Test the longest request you intend to serve, not only an empty prompt.
...
That mechanism can make a larger checkpoint executable, but it puts storage reads inside model execution. Accelerate’s documentation warns that hard-drive offload can be slow when the storage and CPU path cannot move data quickly enough.[^15]

Disk offload is therefore a capacity technique, not extra RAM. It is most defensible when completing the task matters more than interactive token latency, or when only a small fraction of tensors need disk placement. Benchmark with a warm and a cold file cache so an operating-system cache does not hide the storage dependency.
