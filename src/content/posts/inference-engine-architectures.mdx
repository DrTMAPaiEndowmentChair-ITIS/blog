...
topics: ['vLLM', 'SGLang', 'TensorRT-LLM']
---

import EngineLayerStack from '../../components/viz/EngineLayerStack.astro'

Comparisons between inference engines usually take the form of a throughput bar chart on one model at one batch size, which mostly measures which project last tuned that configuration. A more durable way to choose is to ask which layer of the stack each engine treats as its primary design problem, because that choice is stable across releases and it predicts which workloads the engine will handle well.
...
Most published comparisons violate several of these, which is why they disagree with each other and with what teams observe. The layer each engine optimizes is a more stable predictor than any of them.
